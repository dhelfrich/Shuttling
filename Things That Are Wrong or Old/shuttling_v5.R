...
cc <- c(K_s,K_f, K_sf, G, H, T_rbar, T_g, R, Q_n, M, lE_b, lE_r, lE_s, C, K_0, tau, h, maxt)
...
integrate <- function (cc, T_b0, T_a) {
  ...
  for (i in seq(1,length(time_track)-1,by=1)){
    ...
    T_btrack[i+1] <- T_btrack[i]+(1/6)*h*(k1+2*k2+2*k3+k4)
...
T_states <- c(10,35,10)#40*sin(2*(seq(0.0000001,20,by=pi/4)))
